...
class GitOps {
   ...
   */
  async getStagedDiff(root, maxLines = 1000) {
    try {
   ...
   * @returns {Promise<string>} diff 文本
   */
  async getAllDiff(root, maxLines = 1000) {
    try {
  ...
  async commit(root, message) {
    return new Promise((resolve) => {
      execFile('git', ['commit', '-m', message], { cwd: root, maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
        if (err) {
