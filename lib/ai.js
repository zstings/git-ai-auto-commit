const https = require('https');
const http = require('http');
const { URL } = require('url');

// 默认提交规则
const DEFAULT_COMMIT_STYLE_RULE = `你是一个严谨的 Git 提交信息生成助手。请严格遵守以下规则：

## 格式

\`<type>(<scope>): <subject>\`

空一行后接 body（要点式描述）。

## Type 选择规则

- **feat**: 新增用户可感知的功能、页面、组件
- **fix**: 修复已有功能的 Bug（原来能用但出了问题，或行为不符合预期）
- **perf**: 性能优化（减少请求、减少渲染、加快响应，功能不变）
- **refactor**: 重构代码结构（提取函数、调整逻辑组织，功能不变）
- **style**: 文案、代码格式化（不涉及逻辑变更）
- **docs**: 文档、注释变更
- **chore**: 依赖升级、构建配置、工具配置

### 关键区分

- 代码有 Bug 导致功能异常 → fix
- 功能正常但换了更优的实现方式 → perf 或 refactor
- 只改了显示文案/格式 → style
- 新增了之前没有的能力 → feat

## Subject 规则

- 使用中文
- 以动词开头（增加、修复、优化、调整、移除）
- 不超过 50 个字
- 不加句号

## Body 规则

- 与 subject 之间空一行
- 每条以 \`- \` 开头，描述一处具体变更
- 描述"做了什么"，必要时补充"为什么"
- 每条不超过 60 个字
- 按文件/模块/逻辑分组，相关变更合并为一条
- 不重复 subject 已表达的信息
- 不加句号

## 示例

- \`feat(auth): 增加登录功能

- 新增 LoginForm 组件，含账号密码输入与校验
- 接入 /api/auth/login 接口，登录成功后写入 token
- 登录失败时弹出错误提示并保留输入\`
- \`fix(order): 修复金额计算精度丢失问题

- 金额累加改用 decimal.js 避免 JS 浮点误差
- 单元测试覆盖小数累加场景\`
- \`perf(userOrderList): 备注更新后直接修改本地数据避免重新请求

- 备注保存成功后本地 splice 替换对应记录
- 移除保存后的整列表刷新调用\`
- \`refactor(utils): 提取公共校验逻辑为独立函数

- 新增 validateField.js，统一表单字段校验入口
- 原各页面校验逻辑改为调用统一函数\`
- \`style(rebate): 保存按钮文案从"全部保存"改为"保存"

- RebateToolbar 按钮文案改为"保存"\``;

/**
 * AI 引擎：调用 AI API 生成 commit message
 */
class AIClient {
  constructor(config) {
    this.config = config;
  }

  /**
   * 构建 system prompt
   * @param {string} lang - 语言
   * @param {string} [styleRule] - 用户自定义提交规则，不传则使用默认 Conventional Commits 规则
   * @returns {string} system prompt
   */
  buildSystemPrompt(lang, styleRule) {
    const styleGuide = (styleRule && styleRule.trim())
      ? styleRule.trim()
      : DEFAULT_COMMIT_STYLE_RULE;

    const langGuide =
      lang === 'zh'
        ? '提交消息使用中文。'
        : 'Commit message in English.';

    return `你是一个专业的 Git 提交消息生成助手。根据提供的 git diff，生成一条准确的 commit message。

规则：
1. ${styleGuide}
2. ${langGuide}
3. 只返回完整的 commit message（含 subject 行、空行和 body 要点），不要包含任何解释、前后缀或 markdown 代码块标记。
4. 如果 diff 为空或无法分析，返回 "chore: 更新"。`;
  }

  /**
   * 构建 user prompt（含 diff）
   * @param {string} diff - git diff 文本
   * @param {string[]} files - 变更文件列表
   * @returns {string} user prompt
   */
  buildUserPrompt(diff, files) {
    const fileList = files.length > 0 ? `\n\n变更文件列表：\n${files.join('\n')}` : '';
    return `请分析以下 git diff，生成一条完整的 commit message（含 subject 行、空行和 body 要点）：${fileList}\n\n\`\`\`diff\n${diff}\n\`\`\``;
  }

  /**
   * 调用 AI API 生成 commit message（非流式）
   * @param {string} diff - git diff 文本
   * @param {string[]} files - 变更文件列表
   * @returns {Promise<string>} 生成的 commit message
   */
  async generateCommitMessage(diff, files) {
    return this.generateCommitMessageStream(diff, files);
  }

  /**
   * 调用 AI API 生成 commit message（流式）
   * @param {string} diff - git diff 文本
   * @param {string[]} files - 变更文件列表
   * @param {function} [onChunk] - 流式回调，每收到一段文本时调用
   * @returns {Promise<string>} 生成的 commit message
   */
  async generateCommitMessageStream(diff, files, onChunk) {
    const aiConfig = await this.config.getActiveAiConfig();
    const apiKey = aiConfig?.apiKey || '';
    const baseUrl = await this.config.getEffectiveBaseUrl(aiConfig);
    const model = await this.config.getEffectiveModel(aiConfig);
    const apiMode = await this.config.getEffectiveApiMode(aiConfig);
    const lang = await this.config.get('language');
    const styleRule = await this.config.get('commitStyleRule');

    if (!apiKey) {
      throw new Error('未配置 API Key，请在配置页面中添加 AI 配置');
    }
    if (!baseUrl) {
      throw new Error('未配置 API 地址，请在配置页面中填写');
    }
    if (!model) {
      throw new Error('未配置模型名称，请在配置页面中填写');
    }

    const systemPrompt = this.buildSystemPrompt(lang, styleRule);
    const userPrompt = this.buildUserPrompt(diff, files);

    if (apiMode === 'responses') {
      return this.callResponsesApiStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
    }
    if (apiMode === 'anthropic') {
      return this.callAnthropicStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
    }
    return this.callChatCompletionsStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
  }

  /**
   * 调用 Chat Completions API（流式，兼容非流式回退）
   */
  async callChatCompletionsStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    const url = new URL(`${baseUrl}/chat/completions`);
    const body = JSON.stringify({
      model,
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: userPrompt },
      ],
      temperature: 0.3,
      stream: true,
    });

    let fullText = '';
    try {
      fullText = await this.httpRequestStream(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${apiKey}`,
          'Accept-Encoding': 'identity',
        },
        body,
      }, (line) => {
        // 解析 Chat Completions SSE: data: {json}
        if (!line.startsWith('data: ')) return '';
        const jsonStr = line.slice(6).trim();
        if (jsonStr === '[DONE]') return '';
        try {
          const data = JSON.parse(jsonStr);
          const content = data.choices?.[0]?.delta?.content || '';
          if (content && onChunk) onChunk(content);
          return content;
        } catch { return ''; }
      });
    } catch (e) {
      // 流式失败，尝试非流式回退
      console.warn('Chat Completions 流式失败，尝试非流式:', e.message);
    }

    if (!fullText) {
      fullText = await this.callChatCompletionsNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
    }
    return this.cleanMessage(fullText);
  }

  /**
   * 调用 Chat Completions API（非流式回退）
   */
  async callChatCompletionsNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    const url = new URL(`${baseUrl}/chat/completions`);
    const body = JSON.stringify({
      model,
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: userPrompt },
      ],
      temperature: 0.3,
    });

    const data = await this.httpRequest(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${apiKey}`,
        'Accept-Encoding': 'identity',
      },
      body,
    });

    try {
      const json = JSON.parse(data);
      const content = json.choices?.[0]?.message?.content || '';
      if (content && onChunk) onChunk(content);
      if (!content) {
        console.error('Chat Completions 返回为空, 原始响应:', data.slice(0, 500));
        throw new Error('AI 返回为空（原始响应已打印到控制台）');
      }
      return content;
    } catch (e) {
      if (e.message.startsWith('AI 返回为空')) throw e;
      console.error('Chat Completions 解析失败, 原始响应:', data.slice(0, 500));
      throw new Error(`Chat Completions 响应解析失败: ${e.message}`);
    }
  }

  /**
   * 调用 Responses API（流式，兼容非流式回退）
   */
  async callResponsesApiStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    const url = new URL(`${baseUrl}/responses`);
    const body = JSON.stringify({
      model,
      instructions: systemPrompt,
      input: userPrompt,
      temperature: 0.3,
      stream: true,
    });

    let fullText = '';
    try {
      fullText = await this.httpRequestStream(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${apiKey}`,
          'Accept-Encoding': 'identity',
        },
        body,
      }, (line) => {
        // 解析 Responses API SSE
        if (!line.startsWith('data: ')) return '';
        const jsonStr = line.slice(6).trim();
        if (jsonStr === '[DONE]') return '';
        try {
          const data = JSON.parse(jsonStr);
          let text = '';
          // Responses API 流式增量
          if (data.type === 'response.output_text.delta' && data.delta) {
            text = data.delta;
            if (onChunk) onChunk(text);
          }
          // 兼容 Chat Completions 格式
          const chatContent = data.choices?.[0]?.delta?.content || '';
          if (chatContent && onChunk) onChunk(chatContent);
          return text || chatContent;
        } catch { return ''; }
      });
    } catch (e) {
      console.warn('Responses 流式失败，尝试非流式:', e.message);
    }

    if (!fullText) {
      fullText = await this.callResponsesNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
    }
    return this.cleanMessage(fullText);
  }

  /**
   * 调用 Responses API（非流式回退）
   */
  async callResponsesNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    const url = new URL(`${baseUrl}/responses`);
    const body = JSON.stringify({
      model,
      instructions: systemPrompt,
      input: userPrompt,
      temperature: 0.3,
    });

    const data = await this.httpRequest(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${apiKey}`,
        'Accept-Encoding': 'identity',
      },
      body,
    });

    try {
      const json = JSON.parse(data);
      const content = json.output?.[0]?.content?.[0]?.text || json.output_text || json.choices?.[0]?.message?.content || '';
      if (content && onChunk) onChunk(content);
      if (!content) {
        console.error('Responses 返回为空, 原始响应:', data.slice(0, 500));
        throw new Error('AI 返回为空（原始响应已打印到控制台）');
      }
      return content;
    } catch (e) {
      if (e.message.startsWith('AI 返回为空')) throw e;
      console.error('Responses 解析失败, 原始响应:', data.slice(0, 500));
      throw new Error(`Responses 响应解析失败: ${e.message}`);
    }
  }

  /**
   * 调用 Anthropic Messages API（流式，兼容非流式回退）
   */
  async callAnthropicStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    // 智能拼接路径：baseUrl 含 /v1 则直接加 /messages，否则加 /v1/messages
    const messagesPath = baseUrl.endsWith('/v1') || baseUrl.includes('/v1/')
      ? '/messages'
      : '/v1/messages';
    const url = new URL(`${baseUrl}${messagesPath}`);
    const body = JSON.stringify({
      model,
      system: systemPrompt,
      messages: [
        { role: 'user', content: userPrompt },
      ],
      temperature: 0.3,
      stream: true,
      max_tokens: 1024,
    });

    let fullText = '';
    try {
      fullText = await this.httpRequestStream(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': apiKey,
          Authorization: `Bearer ${apiKey}`,
          'anthropic-version': '2023-06-01',
          'Accept-Encoding': 'identity',
        },
        body,
      }, (line) => {
        // 解析 Anthropic SSE: event: xxx\ndata: {json}
        if (!line.startsWith('data: ')) return '';
        const jsonStr = line.slice(6).trim();
        if (jsonStr === '[DONE]') return '';
        try {
          const data = JSON.parse(jsonStr);
          let text = '';
          // Anthropic 标准流式: content_block_delta 事件
          if (data.type === 'content_block_delta' && data.delta?.text) {
            text = data.delta.text;
          }
          // 兼容 OpenAI 格式的流式: choices[0].delta.content
          if (!text) {
            const c = data.choices?.[0]?.delta?.content;
            if (typeof c === 'string') text = c;
          }
          // 兼容部分代理返回的字段
          if (!text) {
            const dt = data.delta?.text;
            const t = data.text;
            const m = data.message;
            if (typeof dt === 'string') text = dt;
            else if (typeof t === 'string') text = t;
            else if (typeof m === 'string') text = m;
          }
          if (text && onChunk) onChunk(text);
          return text;
        } catch { return ''; }
      });
    } catch (e) {
      console.warn('Anthropic 流式失败，尝试非流式:', e.message);
    }

    if (!fullText) {
      fullText = await this.callAnthropicNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk);
    }
    return this.cleanMessage(fullText);
  }

  /**
   * 调用 Anthropic Messages API（非流式回退）
   * 兼容标准 Anthropic 格式、OpenAI 格式、以及各种代理服务返回
   */
  async callAnthropicNonStream(baseUrl, apiKey, model, systemPrompt, userPrompt, onChunk) {
    const messagesPath = baseUrl.endsWith('/v1') || baseUrl.includes('/v1/')
      ? '/messages'
      : '/v1/messages';
    const url = new URL(`${baseUrl}${messagesPath}`);
    const body = JSON.stringify({
      model,
      system: systemPrompt,
      messages: [
        { role: 'user', content: userPrompt },
      ],
      temperature: 0.3,
      max_tokens: 1024,
    });

    const data = await this.httpRequest(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': apiKey,
        Authorization: `Bearer ${apiKey}`,
        'anthropic-version': '2023-06-01',
        'Accept-Encoding': 'identity',
      },
      body,
    });

    let content = '';
    try {
      const json = JSON.parse(data);

      // 1. 标准 Anthropic 格式: { content: [{ type: 'text', text: '...' }] }
      if (Array.isArray(json.content)) {
        for (const block of json.content) {
          if (block.type === 'text' && block.text) {
            content += block.text;
          }
        }
      }

      // 2. OpenAI 格式: { choices: [{ message: { content: '...' } }] }
      if (!content) {
        content = json.choices?.[0]?.message?.content || '';
      }

      // 3. 直接返回文本: { text: '...' } 或 { message: '...' }
      if (!content) {
        content = json.text || json.message || '';
      }

      // 4. 代理返回 detail 等非标准字段
      if (!content && json.detail) {
        throw new Error(`代理服务返回异常: ${json.detail}（请检查 API 地址和 Key 是否正确）`);
      }
    } catch (e) {
      if (e.message.startsWith('代理服务')) throw e;
      console.error('Anthropic 非流式解析失败, 原始响应:', data.slice(0, 500));
      throw new Error(`Anthropic 响应解析失败: ${e.message}`);
    }
    if (content && onChunk) onChunk(content);
    if (!content) {
      console.error('Anthropic 返回为空, 原始响应:', data.slice(0, 500));
      throw new Error('AI 返回为空（原始响应已打印到控制台）');
    }
    return content;
  }

  /**
   * 发送 HTTP 请求（Promise 封装，非流式）
   */
  httpRequest(url, options) {
    return new Promise((resolve, reject) => {
      const transport = url.protocol === 'https:' ? https : http;

      // 确保 Content-Length 被正确设置，避免 Transfer-Encoding: chunked
      const headers = { ...options.headers };
      if (options.body && !headers['Content-Length'] && !headers['content-length']) {
        headers['Content-Length'] = Buffer.byteLength(options.body);
      }

      const reqOptions = {
        method: options.method,
        hostname: url.hostname,
        port: url.port || (url.protocol === 'https:' ? 443 : 80),
        path: url.pathname + url.search,
        headers,
      };

      const req = transport.request(reqOptions, (res) => {
        const chunks = [];
        res.on('data', (chunk) => {
          chunks.push(chunk);
        });
        res.on('end', () => {
          const data = Buffer.concat(chunks).toString('utf8');
          if (res.statusCode >= 400) {
            reject(new Error(`HTTP ${res.statusCode}: ${data.slice(0, 500)}`));
            return;
          }
          resolve(data);
        });
      });

      req.on('error', (err) => {
        reject(new Error(`请求失败: ${err.message}`));
      });

      req.setTimeout(60000, () => {
        req.destroy(new Error('请求超时（60秒无响应）'));
      });

      if (options.body) {
        req.write(options.body);
      }
      req.end();
    });
  }

  /**
   * 发送流式 HTTP 请求，逐行解析 SSE 数据
   * @param {URL} url - 请求 URL
   * @param {object} options - 请求选项 { method, headers, body }
   * @param {function} onLine - 每收到一行 SSE 数据时的回调，返回值会被累加到 fullText
   * @returns {Promise<string>} 拼接后的完整文本
   */
  httpRequestStream(url, options, onLine) {
    return new Promise((resolve, reject) => {
      const transport = url.protocol === 'https:' ? https : http;

      const headers = { ...options.headers };
      if (options.body && !headers['Content-Length'] && !headers['content-length']) {
        headers['Content-Length'] = Buffer.byteLength(options.body);
      }

      const reqOptions = {
        method: options.method,
        hostname: url.hostname,
        port: url.port || (url.protocol === 'https:' ? 443 : 80),
        path: url.pathname + url.search,
        headers,
      };

      let fullText = '';
      let buffer = '';

      const req = transport.request(reqOptions, (res) => {
        if (res.statusCode >= 400) {
          let errData = '';
          res.on('data', (chunk) => (errData += chunk));
          res.on('end', () => {
            reject(new Error(`HTTP ${res.statusCode}: ${errData.slice(0, 500)}`));
          });
          return;
        }

        res.setEncoding('utf8');
        res.on('data', (chunk) => {
          buffer += chunk;
          // SSE 事件以空行（\n\n 或 \r\n\r\n）分隔，确保完整事件才处理
          // 用正则同时匹配 \n\n 和 \r\n\r\n
          const events = buffer.split(/\r?\n\r?\n/);
          buffer = events.pop() || '';

          for (const evt of events) {
            const trimmed = evt.trim();
            if (!trimmed) continue;
            // 一个事件可能包含多行（event: xxx\ndata: {...}）
            // 逐行处理，把每行交给 onLine 回调
            const lines = trimmed.split(/\r?\n/);
            for (const line of lines) {
              const lineTrimmed = line.trim();
              if (!lineTrimmed) continue;
              if (onLine) {
                const text = onLine(lineTrimmed);
                if (text) fullText += text;
              }
            }
          }
        });
        res.on('end', () => {
          // 处理缓冲区剩余数据
          if (buffer.trim() && onLine) {
            const lines = buffer.split(/\r?\n/);
            for (const line of lines) {
              const lineTrimmed = line.trim();
              if (!lineTrimmed) continue;
              const text = onLine(lineTrimmed);
              if (text) fullText += text;
            }
          }
          resolve(fullText);
        });
      });

      req.on('error', (err) => {
        reject(new Error(`请求失败: ${err.message}`));
      });

      // 设置超时（30秒无数据则失败）
      req.setTimeout(30000, () => {
        req.destroy(new Error('请求超时（30秒无响应）'));
      });

      if (options.body) {
        req.write(options.body);
      }
      req.end();
    });
  }

  /**
   * 清理 AI 返回的消息（去除 markdown 代码块标记、多余空白等）
   */
  cleanMessage(content) {
    let msg = content.trim();

    // 去除 markdown 代码块标记
    msg = msg.replace(/^```[\w]*\n?/m, '').replace(/\n?```$/m, '');

    // 去除首尾引号
    msg = msg.replace(/^["'`]+|["'`]+$/g, '');

    return msg.trim();
  }
}

module.exports = AIClient;
module.exports.DEFAULT_COMMIT_STYLE_RULE = DEFAULT_COMMIT_STYLE_RULE;
