...
class ConfigViewProvider {
  ...
  async _testConfig(config) {
    ...
    const urlPath = isClaude ? '/messages' : '/chat/completions';
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .config-item .provider { font-size: 11px; opacity: 0.6; }
  ...
  <div class="section">
    ...
    <div class="form-group">
      ...
      <select id="commitStyle">
        <option value="conventional">Conventional Commits（feat: / fix:）</option>
    ...
    <div class="modal">
      ...
      <div class="form-row">
        <div class="form-group" style="flex:2">
          ...
          <input type="text" id="configBaseUrl" placeholder="https://..." />
        </div>
        <div class="form-group" style="flex:1">
          <label>模型（留空用默认）</label>
          <input type="text" id="configModel" placeholder="模型名" />
        </div>
      </div>
...
<script>
  ...
  function renderPrefs(prefs) {
    document.getElementById('commitStyle').value = prefs.commitStyle || 'conventional';
  ...
  function openAddModal() {
    ...
    document.getElementById('configProvider').value = 'deepseek';
  ...
  function editConfig(id) {
    ...
    document.getElementById('configProvider').value = c.provider;
  ...
  function deleteConfig(id) {
    if (confirm('确定删除这个配置吗？')) {
      vscode.postMessage({ type: 'deleteConfig', data: { id } });
    }
  }
  ...
  function savePrefs() {
    const data = {
      commitStyle: document.getElementById('commitStyle').value,
