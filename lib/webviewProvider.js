...
class ConfigViewProvider {
  ...
  async _testConfig(config) {
    ...
    const urlPath = apiMode === 'responses' ? '/responses' : '/chat/completions';
  ...
  <div class="modal-overlay" id="configModal">
    <div class="modal">
      ...
      <div class="form-group">
        ...
        <div class="hint">OpenAI 部分新模型需 Responses 模式，多数服务商用 Chat Completions</div>
      </div>
  ...
  function renderConfigList() {
    ...
    list.innerHTML = configs.map(c => {
      ...
      return '<div class="config-item ' + (isActive ? 'active' : '') + '" onclick="setActive(\\''+c.id+'\\')">'
        ...
        + '<div class="meta">' + escapeHtml(c.model || '') + (c.apiMode === 'responses' ? ' · Responses' : '') + '</div>'
        + '<div class="actions">'
