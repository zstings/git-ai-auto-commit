...
const { DEFAULT_COMMIT_STYLE_RULE } = require('./ai');
...
class ConfigViewProvider {
  ...
  async _refresh() {
    ...
    this.view.webview.postMessage({
      type: 'refresh',
      data: { configs, activeId, prefs, defaultCommitStyleRule: DEFAULT_COMMIT_STYLE_RULE },
    });
  }
  ...
  <div class="section">
    ...
    <div class="form-group">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <label style="margin:0">提交规则</label>
        <button class="btn btn-secondary" style="padding:2px 8px;font-size:11px" onclick="resetCommitStyleRule()">重置默认</button>
      </div>
    ...
    <div class="form-group">
      ...
      <input type="number" id="maxDiffLines" value="1000" min="50" max="5000" />
    </div>
    <div class="form-group">
...
<script>
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
      case 'refresh': {
        ...
        defaultCommitStyleRule = msg.data.defaultCommitStyleRule || '';
        renderConfigList();
  ...
  function renderPrefs(prefs) {
    ...
    document.getElementById('maxDiffLines').value = prefs.maxDiffLines || 1000;
  ...
  function savePrefs() {
    const data = {
      ...
      maxDiffLines: parseInt(document.getElementById('maxDiffLines').value, 10) || 1000,
  ...
  // ─── 重置提交规则为默认值 ───
  function resetCommitStyleRule() {
    document.getElementById('commitStyleRule').value = defaultCommitStyleRule;
  }
