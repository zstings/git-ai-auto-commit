...
class ConfigManager {
  ...
  /**
   * 获取活跃的 AI 配置对象（含 provider / apiKey / baseUrl / model）
  ...
  async getActiveAiConfig() {
    ...
    return {
      id: 'legacy',
      name: 'Default',
      provider: config.get('apiProvider') || 'deepseek',
  ...
  async getAll() {
    if (this.configProvider) {
      ...
      return {
        apiProvider: activeConfig?.provider || 'deepseek',
  ...
  async getEffectiveBaseUrl(config) {
    ...
    return this.getDefaultBaseUrl(cfg?.provider || 'deepseek');
  }
  ...
  async getEffectiveModel(config) {
    ...
    return this.getDefaultModel(cfg?.provider || 'deepseek');
  }
